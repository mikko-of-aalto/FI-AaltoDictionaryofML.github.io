...
\begin{align}
	&\rawfeaturevec &\quad &\parbox{.85\textwidth}{Instead of the symbol $\featurevec$, we 
		sometimes use $\rawfeaturevec$ as another symbol to denote a \gls{featurevec}. This is the case for example 
		with raw and learned \glspl{feature} \cite[Ch. 9]{MLBasics}.
...
\begin{align} 
	...
	$\localhypothesis{\nodeidx}\in \localmodel{\nodeidx}$ at some node $\nodeidx$ within a \gls{flnetwork}.}\nonumber \\[2mm] \hline \nonumber\\[-5mm]
	& \learntlocalhypothesis{\nodeidx} & \quad & \parbox{.70\textwidth}{A learnt 
	\gls{hypothesis} $\learntlocalhypothesis{\nodeidx}\in \localmodel{\nodeidx}$ some node $\nodeidx$ 
	within a \gls{flnetwork} that has been obtained by some \gls{fl} method.}\nonumber  
\end{align}        
