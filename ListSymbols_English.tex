...
\newpage
\begin{align}
    	&\argmin_{\weights \in \mathcal{C}} f(\weights) &\quad &\parbox{.70\textwidth}{The set $\weights \in \mathcal{C}$ minimizing the real-valued \gls{function} $f: \mathcal{C} \rightarrow \mathbb{R}$. 
...
\begin{align}
	...
	&\bd{\cluster} & \quad & \parbox{.70\textwidth}{\Gls{boundary} of a subset $\cluster$ of some \gls{metricspace}.
		\\ See also: \gls{boundary}, \gls{metricspace}. }   \nonumber
\end{align} 



\subsection*{\Glspl{matrix} and \Glspl{vector}} 
...
\begin{align} 
	...
	&\featurevec\,^{T} &\quad & \parbox{.75\textwidth}{The \gls{transpose} of a \gls{vector} $\featurevec \in \mathbb{R}^{\dimlocalmodel}$ is a \gls{matrix} $\featurevec\,^{T} \in \mathbb{R}^{1 \times \dimlocalmodel}$ with the \gls{vector} as its single row.
		\\ See also: \gls{transpose}, \gls{matrix}, \gls{vector}. } \nonumber 
\end{align} 
...
\begin{align} 
	...
	&\mathbf{A} \otimes \mathbf{B} &\quad & \parbox{.85\textwidth}{The \gls{kroneckerproduct} 
		of \glspl{matrix} $\mathbf{A}$ and $\mathbf{B}$ \cite{Golub1980}.
	...
	&\va \geq \vb &\quad & \parbox{.85\textwidth}{Entrywise inequality between 
		...
		$$a_{\featureidx} \geq b_{\featureidx} \mbox{ for }\featureidx=1,\,\ldots,\,\featuredim.$$
		See also: \gls{vector}. }  \nonumber \\[2mm] \hline \nonumber\\[-5mm]
	&\closedball{\featurevec}{\varepsilon} &\quad & \parbox{.85\textwidth}{Closed ball in some \gls{metricspace} 
		that contains all points with a distance from $\featurevec$ less or equal to $\varepsilon$.
		\\ See also: \gls{metricspace}. } \nonumber
\end{align} 
...
\begin{align}
	&\featurevec \sim \probdist^{(\vz)}  &\quad & \parbox{.85\textwidth}{The \gls{rv} $\featurevec$ is distributed according to 
	...
	&\expect_{p} \{ f(\datapoint) \}  &\quad & \parbox{.85\textwidth}{The \gls{expectation} of an \gls{rv} $f(\datapoint)$, that 
		...
		$\datapoint$ whose \gls{probdist} has a \gls{pdf} $p^{(\datapoint)}$. 
		If the \gls{probdist} is clear from context, we just write $\expect \{ f(\datapoint) \}$. 
		\\ See also: \gls{expectation}, \gls{rv}, \gls{function}, \gls{probdist}, \gls{pdf}.}  \nonumber \\[2mm] \hline \nonumber\\[-5mm]
...
\begin{align} 
	...
	&\samplespace &\quad & \parbox{.85\textwidth}{A \gls{samplespace} of all possible \glspl{outcome} of a \gls{randomexperiment}. 
		\\ See also: \gls{event}.}  \nonumber \\[2mm] \hline \nonumber\\[-5mm]
	&\sigmaalgebra &\quad & \parbox{.85\textwidth}{A collection of \gls{measurable} subsets of a \gls{samplespace} $\samplespace$. % Is this all possible subsets? 
		\\ See also: \gls{samplespace}, \gls{event}.}  \nonumber \\[2mm] \hline \nonumber\\[-5mm]
	&\mathcal{P} &\quad & \parbox{.85\textwidth}{A \gls{probspace} that consists of a \gls{samplespace} $\samplespace$, a  % Should we define it as P := (Sigma, F, P(.))?
...
\begin{align} 
	& {\rm stack} \big\{ \weights^{(\nodeidx)} \big\}_{\nodeidx=1}^{\nrnodes} & \quad & \parbox{.70\textwidth}{The \gls{vector} 
		...
		\\ See also: \gls{vector}, \gls{stacking}, \gls{modelparam}. } \nonumber  
\end{align}        
